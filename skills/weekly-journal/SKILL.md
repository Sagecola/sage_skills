...
1. If the user specifies a week (e.g., "W15", "第 15 周", "上周"), resolve it to `YYYY-Www`.
...
2. If found, read it (~200-500 tokens) and apply its style characteristics.
...
**Token efficiency:**
- First time (create profile): ~6000-8000 tokens
...
- **情绪 tags** (`#开心 #低落` etc.) -> emotion tags are daily-level granularity, not weekly
...
- **明日工作安排** from each day -> only the last day's plan informs `下周计划`
...
- Example: "本周工作主要集中在 X 项目的推进上，完成了 Y 和 Z，并和 [[某人|昵称]] 就 W 做了更深入的讨论。"

**进展 (Progress)**
- Source: "成果和进展" from each day -> extract only the substantive items
- DROP: routine platform updates, trivial logistics
...
- If no genuine improvement items exist, write `- 本周无明显改进事项`
...
- Be honest -> `- 主要是跟着休息` is a valid entry
...
- **If the user's daily journals have a strong "思绪"**: develop it into a callout block:
  ...
  - Use `> [!help]+` for deeper explorations -> when the theme has been building across multiple weeks, or the topic requires quoting external material, structuring a multi-part argument, or going beyond personal feeling into broader questions
  ...
  - Expand the thought: add context, reasoning, and connections to other ideas
...
- **Work section**: prefer bullet lists for `进展 / 改进 / 下周计划`; paragraph for `内容`
...
6. **Emotion must be earned**: Only write emotional language when the daily journals contain the event that earned it
