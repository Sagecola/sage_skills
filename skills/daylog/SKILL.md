...
脚本运行会产生中间文件（`ai-log-index.json`、`ai-worklog.md`、`timeline_YYYY-MM-DD.md` 等），默认写到当前工作目录。这些文件在最终摘要写完后仍然保留，方便用户查看原始数据或二次加工。不需要额外指定输出路径，默认即可。
...
写摘要的原则：
- 每天一个 `## YYYY.M.D` 段落（如 `## 2026.5.26`，无前导零，点分隔）
...
每天一个 `## YYYY.M.D` 段落（如 `## 2026.3.10`，无前导零，点分隔），按时间线用自然语言描述：
